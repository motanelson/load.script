...
print("\033c\033[47;30m\nget me a .log logscript file to view")
a=input().strip()
f1=open(a,"r")
ff=f1.read()
f1.close()
fff=ff.split("\n")
vars=[["print",""],["shell",""],["log","output.txt"],["input",""]]
for b in fff:
    b=b.strip()
    ...
    if b!="":
        ...
        if len(bb)==1:
            ...
            if bb[0]=="print":
                ...
                os.system(finds(vars,"shell"))
            elif bb[0]=="log":
                f1=open(finds(vars,"log"),"a")
                f1.write(finds(vars,"print"))
                f1.close()
            elif bb[0]=="clear":
                f1=open(finds(vars,"log"),"w")
                f1.close()
            elif bb[0]=="input":
                a=input().strip()
                vars=findse(vars,"input",a)
            elif bb[0]=="load":
                f1=open(finds(vars,"log"),"r")
                a=f1.read()
                f1.close()
